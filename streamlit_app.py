"""
ETL Health Monitor — Streamlit Cloud deployment
Dashboard для моніторингу loader_runs з PostgreSQL
"""
...
def chart_retry_effectiveness(summary):
    ...
    fig.add_trace(go.Bar(
        name="Retry failed",
        x=df["loader_name"],
        y=df["retried"] - df["retry_saved"],
        marker_color="#ef4444",
    ))
    fig.update_layout(
        barmode="stack",
        title="Ефективність ретраїв",
        height=300,
        margin=dict(l=10, r=10, t=40, b=10),
    )
    return fig
